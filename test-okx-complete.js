...
async function testMultipleTokens() {
  ...
  if (response.code === '0') {
    ...
    response.data.forEach((item, index) => {
      ...
      console.log(`   ${symbol}: $${price < 0.001 ? price.toExponential(3) : price.toFixed(6)}`);
    });
  } else {
    console.log(`❌ Error: ${response.msg}`);
  }
...
async function testPerformanceMetrics() {
  ...
  if (response.code === '0') {
    ...
    console.log(`   Avg Time per Token: ${(responseTime / response.data.length).toFixed(1)}ms`);
    ...
    console.log(`   Data Freshness: ${dataAge < 30000 ? 'Fresh' : 'Stale'} (${(dataAge / 1000).toFixed(1)}s old)`);
  } else {
...
async function runAllTests() {
  ...
  try {
    ...
    console.log('═'.repeat(60));
    console.log('✅ OKX DEX API is fully functional and ready for production use.');
    console.log('💡 You can now use this API for real-time DEX price data in your application.');
